...
final class XmlReporter implements ReporterInterface
{
    ...
    public function generate(ReportSummary $reportSummary)
    {
        ...
        $ = $dom->createElement('report');
        $dom->appendChild($);

        $filesXML = $dom->createElement('files');
        $->appendChild($filesXML);
        ...
        if (0 !== $reportSummary->getTime()) {
            $->appendChild($this->createTimeElement($reportSummary->getTime(), $dom));
        }

        if (0 !== $reportSummary->getMemory()) {
            $->appendChild($this->createMemoryElement($reportSummary->getMemory(), $dom));
        }
