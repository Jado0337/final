...
class PHPUnit_Util_Configuration
{
    ...
    public function getPHPUnitConfiguration()
    {
        ...
        if ($->hasAttribute('cacheTokens')) {
            $result['cacheTokens'] = $this->getBoolean(
                (string) $->getAttribute('cacheTokens'),
                false
            );
        }

        if ($->hasAttribute('columns')) {
            $columns = (string) $->getAttribute('columns');
        ...
        if ($->hasAttribute('colors')) {
            /* only allow boolean for compatibility with previous versions
              'always' only allowed from command line */
            if ($this->getBoolean($->getAttribute('colors'), false)) {
         ...
         * Issue #657
         */
        if ($->hasAttribute('stderr')) {
            $result['stderr'] = $this->getBoolean(
                (string) $->getAttribute('stderr'),
                false
            );
        }

        if ($->hasAttribute('backupGlobals')) {
            $result['backupGlobals'] = $this->getBoolean(
                (string) $->getAttribute('backupGlobals'),
                true
            );
        }

        if ($->hasAttribute('backupStaticAttributes')) {
            $result['backupStaticAttributes'] = $this->getBoolean(
                (string) $->getAttribute('backupStaticAttributes'),
                false
            );
        }

        if ($->getAttribute('bootstrap')) {
            $result['bootstrap'] = $this->toAbsolutePath(
                (string) $->getAttribute('bootstrap')
            );
        }

        if ($->hasAttribute('convertErrorsToExceptions')) {
            $result['convertErrorsToExceptions'] = $this->getBoolean(
                (string) $->getAttribute('convertErrorsToExceptions'),
                true
            );
        }

        if ($->hasAttribute('convertNoticesToExceptions')) {
            $result['convertNoticesToExceptions'] = $this->getBoolean(
                (string) $->getAttribute('convertNoticesToExceptions'),
                true
            );
        }

        if ($->hasAttribute('convertWarningsToExceptions')) {
            $result['convertWarningsToExceptions'] = $this->getBoolean(
                (string) $->getAttribute('convertWarningsToExceptions'),
                true
            );
        }

        if ($->hasAttribute('forceCoversAnnotation')) {
            $result['forceCoversAnnotation'] = $this->getBoolean(
                (string) $->getAttribute('forceCoversAnnotation'),
                false
            );
        }

        if ($->hasAttribute('mapTestClassNameToCoveredClassName')) {
            $result['mapTestClassNameToCoveredClassName'] = $this->getBoolean(
                (string) $->getAttribute('mapTestClassNameToCoveredClassName'),
                false
            );
        }

        if ($->hasAttribute('processIsolation')) {
            $result['processIsolation'] = $this->getBoolean(
                (string) $->getAttribute('processIsolation'),
                false
            );
        }

        if ($->hasAttribute('stopOnError')) {
            $result['stopOnError'] = $this->getBoolean(
                (string) $->getAttribute('stopOnError'),
                false
            );
        }

        if ($->hasAttribute('stopOnFailure')) {
            $result['stopOnFailure'] = $this->getBoolean(
                (string) $->getAttribute('stopOnFailure'),
                false
            );
        }

        if ($->hasAttribute('stopOnIncomplete')) {
            $result['stopOnIncomplete'] = $this->getBoolean(
                (string) $->getAttribute('stopOnIncomplete'),
                false
            );
        }

        if ($->hasAttribute('stopOnRisky')) {
            $result['stopOnRisky'] = $this->getBoolean(
                (string) $->getAttribute('stopOnRisky'),
                false
            );
        }

        if ($->hasAttribute('stopOnSkipped')) {
            $result['stopOnSkipped'] = $this->getBoolean(
                (string) $->getAttribute('stopOnSkipped'),
                false
            );
        }

        if ($->hasAttribute('testSuiteLoaderClass')) {
            $result['testSuiteLoaderClass'] = (string) $->getAttribute(
                'testSuiteLoaderClass'
            );
        }

        if ($->getAttribute('testSuiteLoaderFile')) {
            $result['testSuiteLoaderFile'] = $this->toAbsolutePath(
                (string) $->getAttribute('testSuiteLoaderFile')
            );
        }

        if ($->hasAttribute('printerClass')) {
            $result['printerClass'] = (string) $->getAttribute(
                'printerClass'
            );
        }

        if ($->getAttribute('printerFile')) {
            $result['printerFile'] = $this->toAbsolutePath(
                (string) $->getAttribute('printerFile')
            );
        }

        if ($->hasAttribute('timeoutForSmallTests')) {
            $result['timeoutForSmallTests'] = $this->getInteger(
                (string) $->getAttribute('timeoutForSmallTests'),
                1
            );
        }

        if ($->hasAttribute('timeoutForMediumTests')) {
            $result['timeoutForMediumTests'] = $this->getInteger(
                (string) $->getAttribute('timeoutForMediumTests'),
                10
            );
        }

        if ($->hasAttribute('timeoutForLargeTests')) {
            $result['timeoutForLargeTests'] = $this->getInteger(
                (string) $->getAttribute('timeoutForLargeTests'),
                60
            );
        }

        if ($->hasAttribute('beStrictAboutTestsThatDoNotTestAnything')) {
            $result['reportUselessTests'] = $this->getBoolean(
                (string) $->getAttribute('beStrictAboutTestsThatDoNotTestAnything'),
                false
            );
        }

        if ($->hasAttribute('checkForUnintentionallyCoveredCode')) {
            $result['strictCoverage'] = $this->getBoolean(
                (string) $->getAttribute('checkForUnintentionallyCoveredCode'),
                false
            );
        }

        if ($->hasAttribute('beStrictAboutOutputDuringTests')) {
            $result['disallowTestOutput'] = $this->getBoolean(
                (string) $->getAttribute('beStrictAboutOutputDuringTests'),
                false
            );
        }

        if ($->hasAttribute('beStrictAboutChangesToGlobalState')) {
            $result['disallowChangesToGlobalState'] = $this->getBoolean(
                (string) $->getAttribute('beStrictAboutChangesToGlobalState'),
                false
            );
        }

        if ($->hasAttribute('beStrictAboutTestSize')) {
            $result['enforceTimeLimit'] = $this->getBoolean(
                (string) $->getAttribute('beStrictAboutTestSize'),
                false
            );
        }

        if ($->hasAttribute('beStrictAboutTodoAnnotatedTests')) {
            $result['disallowTodoAnnotatedTests'] = $this->getBoolean(
                (string) $->getAttribute('beStrictAboutTodoAnnotatedTests'),
                false
            );
        }
        ...
        if ($->hasAttribute('verbose')) {
            $result['verbose'] = $this->getBoolean(
                (string) $->getAttribute('verbose'),
                false
            );
        }
