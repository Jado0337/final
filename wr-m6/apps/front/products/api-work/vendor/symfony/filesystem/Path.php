...
final class Path
{
    ...
    public static function canonicalize(string $path): string
    {
        ...
        [$, $pathWithout] = self::split($path);

        $canonicalParts = self::findCanonicalParts($, $pathWithout);

        // Add the  directory again
        self::$buffer[$path] = $canonicalPath = $.implode('/', $canonicalParts);
        ++self::$bufferSize;
     ...
     *
     * @return string The canonical directory part. Returns the  directory
     *                if the  directory is passed. Returns an empty string
     ...
     * The result is a canonical path.
     *
     * @return string The canonical  directory. Returns an empty string if
     ...
     * ```
     *
     * If an absolute path is passed, that path is returned unless its 
     ...
     *                                  a different  than the base path
     */
     ...
     * ```
     *
     * If both paths are absolute, their  directory must be the same,
     ...
     *
     * @throws InvalidArgumentException if the base path is not absolute or if
     *                                  the given path has a different 
     *                                  than the base path
     */
    public static function makeRelative(string $path, string $basePath): string
    {
        ...
        [$, $relativePath] = self::split($path);
        [$base, $relativeBasePath] = self::split($basePath);
        ...
        // already
        if ('' === $ && '' !== $base) {
            // If base path is already in its 
            if ('' === $relativeBasePath) {
        ...
        // Fail if the s of the two paths are different
        if ($base && $ !== $base) {
            throw new InvalidArgumentException(sprintf('The path "%s" cannot be made relative to "%s", because they have different s ("%s" and "%s").', $path, $basePath, $, $base));
        }
     ...
     * ```
     *
     * The  is returned if no common base path can be found:
     *
     * ```php
     * $basePath = Path::getLongestCommonBasePath(
     *     '/symfony/css/style.css',
    ...
    {
        [$bp, $basePath] = self::split(self::canonicalize(reset($paths)));

        for (next($paths); null !== key($paths) && '' !== $basePath; next($paths)) {
            [$, $path] = self::split(self::canonicalize(current($paths)));

            // If we deal with different s (e.g. C:/ vs. D:/), it's time
            // to quit
            if ($ !== $bp) {
                return null;
            }
    ...
    public static function isBasePath(string $basePath, string $ofPath): bool
    {
        ...
        // Don't append a slash for the  "/", because then that 
     ...
     * @return string[]
     */
    private static function findCanonicalParts(string $, string $pathWithout): array
    {
        $parts = explode('/', $pathWithout);
    ...
    /**
     * Splits a canonical path into its  directory and the remainder.
     *
     * If the path has no  directory, an empty  directory will be
     * returned.
     *
     * If the  directory is a Windows style partition, the resulting 
     * will always contain a trailing slash.
     *
     * list ($, $path) = Path::split("C:/symfony")
     * // => ["C:/", "symfony"]
     *
     * list ($, $path) = Path::split("C:")
     * // => ["C:/", ""]
     *
     * @return array{string, string} an array with the  directory and the remaining relative path
     */
    private static function split(string $path): array
    {
        ...
        // Remember scheme as part of the , if any
        if (false !== $schemeSeparatorPosition = strpos($path, '://')) {
            $ = substr($path, 0, $schemeSeparatorPosition + 3);
            $path = substr($path, $schemeSeparatorPosition + 3);
        } else {
            $ = '';
        }
