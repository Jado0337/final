...
(function($, window, document, undefined) {
    ...
    function Plugin(element, options) {
        ...
        if (options.Class !== undefined && options.Class !== 'dd') {
            options.listClass       = options.listClass ? options.listClass : options.Class + '-list';
            options.itemClass       = options.itemClass ? options.itemClass : options.Class + '-item';
            options.dragClass       = options.dragClass ? options.dragClass : options.Class + '-dragel';
            options.handleClass     = options.handleClass ? options.handleClass : options.Class + '-handle';
            options.collapsedClass  = options.collapsedClass ? options.collapsedClass : options.Class + '-collapsed';
            options.placeClass      = options.placeClass ? options.placeClass : options.Class + '-placeholder';
            options.noDragClass     = options.noDragClass ? options.noDragClass : options.Class + '-nodrag';
            options.noChildrenClass = options.noChildrenClass ? options.noChildrenClass : options.Class + '-nochildren';
            options.emptyClass      = options.emptyClass ? options.emptyClass : options.Class + '-empty';
        }
    ...
    Plugin.prototype = {
        ...
        setIndexOfItem: function(item, index) {
            ...
            if ($(item[0].parentNode)[0] !== this.dragEl[0]) {
        ...
        dragStop: function(e) {
            ...
            if (this.hasNew) {
                ...
                this.dragEl.trigger('gainedItem');
            }
            else {
                this.dragEl.trigger('change');
            }

            this.options.callback.call(this, this.dragEl, el, position);
        ...
        dragMove: function(e) {
            ...
            var pointEl = this.pointEl.closest('.' + opt.Class),
                isNew = this.dragEl.data('nestable-id') !== pointEl.data('nestable-id');

            /**
             * move vertical
             */
            if (!mouse.dirAx || isNew || isEmpty) {
                // check if groups match if dragging over new 
                if (isNew && opt.group !== pointEl.data('nestable-group')) {
                    return;
                }
                ...
                if (!this.dragEl.find(opt.itemNodeName).length) {
                    this.appendEmptyElement(this.dragEl);
                }
                // parent  list has changed
                this.dragEl = pointEl;
                if (isNew) {
                    this.hasNew = this.el[0] !== this.dragEl[0];
                }
            }
        },
