...
define("tinymce/inlite/alien/Unlink", [
	...
	var getSelectedElements = function (Elm, startNode, endNode) {
		var walker, node, elms = [];

		walker = new TreeWalker(startNode, Elm);
	...
	var getSelectedAnchors = function (editor) {
		var startElm, endElm, Elm, anchorElms, selection, dom, rng;
		...
		Elm = editor.getBody();
		anchorElms = Tools.grep(getSelectedElements(Elm, startElm, endElm), isLink);
...
define('tinymce/inlite/core/Measure', [
	...
	var getElementRect = function (editor, elm) {
		...
		if (.nodeName == 'BODY') {
			targetRect.x -= .ownerDocument.documentElement.scrollLeft || .scrollLeft;
			targetRect.y -= .ownerDocument.documentElement.scrollTop || .scrollTop;
		}
