...
$.fn.elfindertree = function(fm, opts) {
	...
	this.not('.'+treeclass).each(function() {
		...
		var c = 'class', mobile = fm.UA.Mobile,
			...
			/**
			 * Open  dir if not opened yet
			 *
			 * @type Boolean
			 */
			open  = opts.openOnLoad,
			 ...
			 * @type String
			 */
			wrapper = 'elfinder-navbar-wrapper-',

			/**
			 * Un-disabled cmd `paste` volume's  wrapper class
			 * 
			 * @type String
			 */
			...
			/**
			 * Un-disabled cmd `upload` volume's  wrapper class
			 * 
			 * @type String
			 */
			...
			replace = {
				...
				cssclass    : function(dir) {
					var cname = (dir.phash && ! dir.is ? '' : )+' '+navdir+' '+fm.perms2class(dir);
			...
			/**
			 * Get hashes array of items of the bottom of the leaf  back from the target
			 * 
				...
				var cwd     = fm.cwd(),
					...
					loadParents = function(dir) {
						...
						reqs = $.map(ends, function(h) {
							...
							if (!node.hasClass(loaded) && (is || !d || !fm.navHash2Elm(d.phash).hasClass(loaded))) {
								if (is || closest === getPhash(h) || closest === getPhash(h, 2)) {
									until = void(0);
									cmd = 'tree';
									if (!is) {
										h = getPhash(h);
			...
			/**
			 * Make writable and not  dirs droppable
			 *
			 * @return void
			 */
			updateDroppable = function(target, node) {
				...
				if (!target) {
					if (!node || node.closest('div.'+wrapper).hasClass(uploadable)) {
						(node || tree.find('div.'+uploadable)).find(selNavdir+':not(.elfinder-ro,.elfinder-na)').addClass('native-droppable');
					}
					if (!node || node.closest('div.'+wrapper).hasClass(pastable)) {
					...
					if (node) {
						// check leaf s
						node.children('div.'+wrapper).each(function() {
				...
				$.each(dirs, function(i, dir) {
					fm.navHash2Elm(dir.phash).filter(sel)
						.filter(function() { return $.grep($(this).next('.'+subtree).children(), function(n) {
							return ($(n).children().hasClass())? false : true;
						}).length > 0; })
						.addClass(cls);
				});
			},
			...
			tree = $(this).addClass(treeclass)
				...
				.on('mouseenter mouseleave', selNavdir, function(e) {
					...
					if (!link.hasClass(dropover+' '+disabled)) {
						if (!mobile && enter && !link.data('dragRegisted') && !link.hasClass(+' '+draggable+' elfinder-na elfinder-wo')) {
					...
					if (enter && opts.attrTitle) {
						...
						if (!dir.is && link.attr('title') === (dir.i18 || dir.name)) {
		...
		.bind('sortchange', function() {
			if (fm.sortAlsoTreeview || prevSortTreeview !== fm.sortAlsoTreeview) {
				...
				fm.lazy(function() {
					...
					// append volume s at first
					updateTree($.map(fm.s, function(h) {
						var dir = fm.file(h);
					...
					if (!Object.keys(hasMoreDirs).length) {
						...
						if (ends.length > 1) {
							$.each(ends, function(i, end) {
								var vid = fm.file(fm.(end)).volumeid; 
								if (i === 0) {
									topVid = vid;
								}
								endsVid[vid] = end;
