...
$.fn.elfinderpath = function(fm, options) {
	...
	return this.each(function() {
		var query  = '',
			...
			s = $('<div class="elfinder-path-s"></div>').on('click', function(e) {
				e.stopPropagation();
				e.preventDefault();
				
				var s = $.map(fm.s, function(h) { return fm.file(h); }),
				raw = [];

				$.each(s, function(i, f) {
					if (! f.phash && fm.(fm.cwd().hash, true) !== f.hash) {
						raw.push({
