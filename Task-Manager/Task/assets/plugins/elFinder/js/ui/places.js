...
$.fn.elfinderplaces = function(fm, opts) {
	...
	return this.each(function() {
		var dirs      = {},
			...
			// sort button
			sortBtn = $('<span class="elfinder-button-icon elfinder-button-icon-sort elfinder-places--icon" title="'+fm.i18n('cmdsort')+'"></span>')
				.hide()
				.on('click', function(e) {
					e.stopPropagation();
					subtree.empty();
			...
			places = $(this).addClass(fm.res(c, 'tree')+' elfinder-places ui-corner-all')
				...
				.on('contextmenu', '.'+navdir+':not(.'+cl+')', function(e) {
					var self = $(this),
				...
				// for touch device
				.on('touchstart', '.'+navdir+':not(.'+cl+')', function(e) {
				...
				.on('touchmove touchend', '.'+navdir+':not(.'+cl+')', function(e) {
