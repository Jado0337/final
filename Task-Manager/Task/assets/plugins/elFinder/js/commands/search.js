		...
		if (typeof q == 'string' && q) {
			...
			if (! onlyMimes.length || mime.length) {
				if (target === '' && fm.api >= 2.1) {
					Cnt = Object.keys(fm.s).length;
					$.each(fm.s, function(id, hash) {
						reqDef.push(fm.request({
							data   : setType({cmd : 'search', q : q, target : hash, mimes : mime}),
							notify : {type : 'search', cnt : 1, hideCnt : (Cnt > 1? false : true)},
							cancel : true,
							preventDone : true
						}));
					});
					...
					if (target !== '' && fm.api >= 2.1 && Object.keys(fm.leafs).length) {
						$.each(fm.leafs, function(hash, s) {
							phash = hash;
							while(phash) {
								if (target === phash) {
									$.each(s, function() {
