...
elFinder.prototype.commands.rename = function() {
	...
	// set alwaysEnabled to allow  rename on client size
	...
	var self = this,
		...
		request = function(dfrd, targtes, file, name) {
			...
			if (fm.is(file) && !file.netkey) {
				if (!(Names = fm.storage('Names'))) {
					Names = {};
				}
				if (name === '') {
					if (Names[file.hash]) {
					...
					file.name = Names[file.hash] = name;
					delete file.i18;
				}
				fm.storage('Names', Names);
				data = { changed: [file] };
				fm.updateCache(data);
				fm.change(data);
	...
	this.getstate = function(select) {
		...
		state = (cnt === 1 && ((fm.cookieEnabled && is) || !sel[0].locked) || (fm.api > 2.1030 && cnt === $.grep(sel, function(f) {
			if (!brk && !f.locked && f.phash === phash && !fm.is(f) && (mime === f.mime || ext === fm.splitFileExtention(f.name)[1].toLowerCase())) {
				return true;
			} else {
				brk && (brk = true);
				return false;
		...
		if (!is && state === 0 && fm.option('disabledFlip', sel[0].hash)['rename']) {
			state = -1;
		}
	...
	fm.bind('select contextmenucreate closecontextmenu', function(e) {
		var sel = (e.data? (e.data.selected || e.data.targets) : null) || fm.selected(),
			file;
		if (sel && sel.length === 1 && (file = fm.file(sel[0])) && fm.is(file)) {
		...
		var Names;
		if (e.data && e.data.removed && (Names = fm.storage('Names'))) {
