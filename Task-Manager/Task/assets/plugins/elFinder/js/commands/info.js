	...
	this.exec = function(hashes) {
		...
		if (cnt === 1) {
			...
			if (!hideItems.link && file.read) {
				...
				if (file.url == '1') {
					...
					if (file.url) {
						...
						} else if (file.url !== '' && fm.option('url', (!fm.is(file) && file.phash) || file.hash)) {
			...
			if (!dcnt) {
				...
				rdcnt = $.grep(files, function(f) { return f.mime === 'directory' && (! f.phash || f.is)? true : false ; }).length;
				dcnt -= rdcnt;
				content.push(row.replace(l, msg.kind).replace(v, (rdcnt === cnt || dcnt === cnt)? msg[rdcnt? 's' : 'folders'] : $.map({s: rdcnt, folders: dcnt, files: cnt - rdcnt - dcnt}, function(c, t) { return c? msg[t]+' '+c : null; }).join(', ')));
