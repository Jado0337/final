...
elFinder.prototype.commands.rm = function() {
		...
		getTHash = function(targets) {
			...
			if (targets && targets.length) {
				if (targets.length > 1 && fm.searchStatus.state === 2) {
					1st = fm.file(fm.(targets[0])).volumeid;
					if (!$.grep(targets, function(h) { return h.indexOf(1st) !== 0? true : false ; }).length) {
	...
	this.getstate = function(select) {
		...
		return sel.length && $.grep(sel, function(h) { var f = fm.file(h); return f && ! f.locked && ! fm.is(f)? true : false; }).length == sel.length
			? 0 : -1;
	};
