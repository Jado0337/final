...
var elFinder = function(elm, opts, bootCallback) {
				...
				rm      = function(hash) {
					...
					if (file) {
						if (file.mime === 'directory') {
							if (s[hash]) {
								delete self.s[s[hash]];
							}
							// restore stats of deleted  parent directory
							$.each(self.leafs, function(phash, s) {
								var idx, pdir;
								if ((idx = $.inArray(hash, s))!== -1) {
									if (s.length === 1) {
										...
										delete self.leafs[phash];
									} else {
										self.leafs[phash].splice(idx, 1);
									}
								}
							});
	...
	/**
	 * Volume option to set the properties of the  Stat
	 * 
	 * @type Object
	 */
	...
	*/
	...
	this.is = function(file) {
		return (file.is || ! file.phash)? true : false;
	};
	
	/**
	 * Return  dir hash for current working directory
	 * 
	 * @param  String   target hash
	 * @param  Boolean  include fake parent (optional)
	 * @return String
	 */
	this. = function(hash, fake) {
		hash = hash || cwd;
		var dir, i;
		
		if (! fake) {
			$.each(self.s, function(id, rhash) {
		...
		dir = files[hash];
		while (dir && dir.phash && (fake || ! dir.is)) {
			dir = files[dir.phash];
		}
		if (dir) {
			return dir.hash;
	...
	this.url = function(hash, o) {
		var file   = files[hash],
			...
			getBaseUrl = function() {
				return self.option('url', (!self.is(file) && file.phash) || file.hash);
			},
			baseUrl, res;
			...
			success = function(response) {
				...
				if (!response) {
					...
					if (isOpen) {
						// check leafs
						$.each(self.leafs, function(phash, s) {
							self.leafs[phash] = $.grep(s, function(h) { return h !== data.target; });
						});
					}
				...
				var resolve = function() {
					var pushLeafs = function(name) {
						if (self.leafs[data.target] && response[name]) {
							$.each(self.leafs[data.target], function(i, h) {
								var ;
								if ( = self.file(h)) {
									response[name].push();
								}
			...
			request = function(mode) {
				...
				dfrd.always(function() {
					...
					if (isOpen) {
						...
						openDir && openDir.volumeid && self.is(openDir) && delete self.volumeExpires[openDir.volumeid];
					}
	...
	this.sync = function(onlydir, polling) {
		...
		var self  = this,
			...
			exParents = function() {
				var parents = [],
					cur = self.file(self.(cwd)),
					curId = cur? cur.volumeid : null,
		...
		if (! onlydir && self.api >= 2) {
			(cwd !== this.()) && opts.push(this.request({
		...
		.fail(function(error, xhr) {
			...
			if (! polling || $.inArray('errOpen', error) !== -1) {
				...
				self.parseError(error) && self.request({
					data   : {cmd : 'open', target : (self.lastDir('') || self.()), tree : 1, init : 1},
...
elFinder.prototype = {
						...
						$.each(files, function(i, file) {
							...
							if (excludes && file.name.match(excludes)) {
								...
								if (relPath && $.inArray(relPath, mkdirs) === -1) {
									mkdirs.push(relPath);
									// checking the  directory to supports <input type="file" webkitdirectory> see #2378
									idx = relPath.substr(1).indexOf('/');
									if (idx !== -1 && (Dir = relPath.substr(0, idx + 1)) && $.inArray(Dir, mkdirs) === -1) {
										mkdirs.unshift(Dir);
									}
								}
							}
	...
	normalize : function(data) {
		var self   = this,
			...
			filter = function(file, asMap, type) { 
				...
				if (file && file.hash && file.name && file.mime) {
					...
					is = self.is(file);
					if (is && ! file.volumeid) {
						self.debug('warning', 'The volume  statuses requires `volumeid` property.');
					}
					if (is || file.mime === 'directory') {
						...
						if (file.phash) {
							...
							if (is && file.volumeid && file.phash.indexOf(file.volumeid) === 0) {
						...
						if (file.volumeid) {
							...
							if (is) {
								// make or update of leaf s cache
								if (file.phash) {
									if (! self.leafs[file.phash]) {
										self.leafs[file.phash] = [ file.hash ];
									} else {
										if ($.inArray(file.hash, self.leafs[file.phash]) === -1) {
											self.leafs[file.phash].push(file.hash);
										}
									}
								}
						...
						if (is) {
							if (Names = self.storage('Names')) {
								if (Names[file.hash]) {
									file._name = file.name;
									file._i18 = file.i18;
									file.name = Names[file.hash] = Names[file.hash];
									delete file.i18;
								}
								self.storage('Names', Names);
							}
						}
			...
			applyLeafStats = function(dataArr, type) {
				$.each(dataArr, function(i, f) {
					var pfile, done;
					if (self.leafs[f.hash]) {
						self.applyLeafStats(f);
					}
					// update leaf  parent stat
					if (type !== 'change' && f.phash && self.is(f) && (pfile = self.file(f.phash))) {
						self.applyLeafStats(pfile);
						// add to data.changed
						if (!data.changed) {
							data.changed = [pfile];
						} else {
		...
		if (data.cwd) {
			if (data.cwd.volumeid && data.options && Object.keys(data.options).length && self.is(data.cwd)) {
				self.hasVolOptions = true;
		...
		if (Object.keys(self.leafs).length) {
			data.files && applyLeafStats(data.files);
			data.tree && applyLeafStats(data.tree);
			data.added && applyLeafStats(data.added);
			data.changed && applyLeafStats(data.changed, 'change');
		}
	...
	mime2kind : function(f) {
		...
		if (isObj && f.alias && mime != 'symlink-broken') {
			...
			if (isObj && mime === 'directory' && (! f.phash || f.is)) {
				kind = '';
			} else {
	 ...
	 * @param  String   name
	 * @param  String   host
	 * @param  Object   opts  Default {noOffline: false, : '', pathI18n: 'folderId', folders: true}
			}
	 * 
	 * @return Object
	 */
	makeNetmountOptionOauth : function(protocol, name, host, opt) {
		...
		return {
			...
			inputs: {
				...
				path     : $('<input type="text" value="'+opts.+'"/>'),
	...
	convAbsUrl : function(url) {
		...
		var  = window.location.protocol + '//' + window.location.host,
			reg  = /[^\/]+\/\.\.\//,
			ret;
		if (url.substr(0, 1) === '/') {
			ret =  + url;
		} else {
			ret =  + window.location.pathname.replace(/\/[^\/]+$/, '/') + url;
		}
		ret = ret.replace('/./', '/');
		while(reg.test(ret)) {
			ret = ret.replace(reg, '');
	...
	getSize : function(targets) {
		var self = this,
			...
			getLeafs = function(file) {
				var targets = [];
				if (file.mime === 'directory') {
					$.each(self.leafs, function(hash, s) {
						var phash;
						if (hash === file.hash) {
							targets.push.apply(targets, s);
						} else {
			...
			cache = function() {
				var dfd = $.Deferred(),
					cnt = Object.keys(self.leafs).length;
				
				if (cnt > 0) {
					$.each(self.leafs, function(hash) {
		...
		cache().done(function() {
			...
			$.each(targets, function() {
				files.push.apply(files, getLeafs(self.file(this)));
			});
	...
	/**
	 * Apply leaf  stats to target directory
	 *
	 ...
	 */
	applyLeafStats : function(dir, update) {
		var self = this,
		...
		// set ts
		$.each(self.leafs[dir.hash], function() {
