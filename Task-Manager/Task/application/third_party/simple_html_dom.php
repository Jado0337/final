...
class simple_html_dom_node
{
     ...
     * match was found.
     *
     * @todo Null is returned implicitly by calling ->parent on the  node.
    ...
    function outertext()
    {
        ...
        if ($this->tag === '') return $this->innertext();
    ...
    function find($selector, $idx = null, $lowercase = false)
    {
        ...
        for ($c = 0; $c < $count; ++$c) {
            ...
            for ($l = 0; $l < $levle; ++$l) {
                ...
                foreach ($head as $k => $v) {
                    $n = ($k === -1) ? $this->dom-> : $this->dom->nodes[$k];
...
class simple_html_dom
{
        ...
        $options = 0)
    {
        ...
        $this->parse();
        // end
        $this->->_[HDOM_INFO_END] = $this->cursor;
        $this->parse_charset();
    ...
    function find($selector, $idx = null, $lowercase = false)
    {
        return $this->->find($selector, $idx, $lowercase);
    }
        ...
        $defaultSpanText = DEFAULT_SPAN_TEXT)
    {
        ...
        $this-> = new simple_html_dom_node($this);
        $this->->tag = '';
        $this->->_[HDOM_INFO_BEGIN] = -1;
        $this->->nodetype = HDOM_TYPE_;
        $this->parent = $this->;
    ...
    protected function parse_charset()
    {
        ...
        if (empty($charset)) {
            $el = $this->->find('meta[http-equiv=Content-Type]', 0, true);
