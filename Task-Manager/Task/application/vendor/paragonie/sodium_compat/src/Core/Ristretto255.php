...
class ParagonIE_Sodium_Core_Ristretto255 extends ParagonIE_Sodium_Core_Ed25519
{
    ...
    public static function ristretto255_sqrt_ratio_m1(
        ...
        $m__check = self::fe_sub($vxx, $u); /* vx^2-u */
        $p__check = self::fe_add($vxx, $u); /* vx^2+u */
        $f__check = self::fe_mul($u, $sqrtm1); /* u*sqrt(-1) */
        $f__check = self::fe_add($vxx, $f__check); /* vx^2+u*sqrt(-1) */

        $has_m_ = self::fe_iszero($m__check);
        $has_p_ = self::fe_iszero($p__check);
        $has_f_ = self::fe_iszero($f__check);

        $x_sqrtm1 = self::fe_mul($x, $sqrtm1); /* x*sqrt(-1) */

        $x = self::fe_abs(
            self::fe_cmov($x, $x_sqrtm1, $has_p_ | $has_f_)
        );
        return array(
            'x' => $x,
            'nonsquare' => $has_m_ | $has_p_
        );
    }
