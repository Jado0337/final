...
class QRcode {
	...
	* Return block number 1
	 ...
	 * @param $fcr (int)  first  of RS code generator polynomial, index form
	 * @param $prim (int)  primitive element to generate polynomial s
	 * @param $ns (int) RS code generator polynomial degree (number of s)
	 * @param $pad (int)  padding bytes at front of shortened block
	 * @return array Array of RS values:<ul><li>mm = Bits per symbol;</li><li>nn = Symbols per block;</li><li>alpha_to = log lookup table array;</li><li>index_of = Antilog lookup table array;</li><li>genpoly = Generator polynomial array;</li><li>ns = Number of generator;</li><li>s = number of parity symbols;</li><li>fcr = First consecutive , index form;</li><li>prim = Primitive element, index form;</li><li>iprim = prim-th  of 1, index form;</li><li>pad = Padding bytes in shortened block;</li><li>gfpoly</ul>.
	 */
	 protected function init_rs($symsize, $gfpoly, $fcr, $prim, $ns, $pad) {
		foreach ($this->rsitems as $rs) {
			if (($rs['pad'] != $pad) OR ($rs['ns'] != $ns) OR ($rs['mm'] != $symsize)
				OR ($rs['gfpoly'] != $gfpoly) OR ($rs['fcr'] != $fcr) OR ($rs['prim'] != $prim)) {
				continue;
			}
			return $rs;
		}
		$rs = $this->init_rs_char($symsize, $gfpoly, $fcr, $prim, $ns, $pad);
	 ...
	 * @return array Array of RS values:<ul><li>mm = Bits per symbol;</li><li>nn = Symbols per block;</li><li>alpha_to = log lookup table array;</li><li>index_of = Antilog lookup table array;</li><li>genpoly = Generator polynomial array;</li><li>ns = Number of generator;</li><li>s = number of parity symbols;</li><li>fcr = First consecutive , index form;</li><li>prim = Primitive element, index form;</li><li>iprim = prim-th  of 1, index form;</li><li>pad = Padding bytes in shortened block;</li><li>gfpoly</ul>.
	 */
	protected function init_rs_char($symsize, $gfpoly, $fcr, $prim, $ns, $pad) {
		...
		if (($ns < 0) OR ($ns >= (1<<$symsize))) {
			return $rs;
		}
		if (($pad < 0) OR ($pad >= ((1<<$symsize) -1 - $ns))) {
			return $rs;
		}
		$rs = array();
		$rs['mm'] = $symsize;
		...
		// Form RS code generator polynomial from its s
		$rs['genpoly'] = array_fill(0, ($ns + 1), 0);
		$rs['fcr'] = $fcr;
		$rs['prim'] = $prim;
		$rs['ns'] = $ns;
		$rs['gfpoly'] = $gfpoly;
		// Find prim-th  of 1, used in decoding
		for ($iprim=1; ($iprim % $prim) != 0; $iprim += $rs['nn']) {
			; // intentional empty-body loop!
		}
		$rs['iprim'] = (int)($iprim / $prim);
		$rs['genpoly'][0] = 1;
		for ($i = 0,$=$fcr*$prim; $i < $ns; $i++, $ += $prim) {
			$rs['genpoly'][$i+1] = 1;
			// Multiply rs->genpoly[] by  @**( + x)
			for ($j = $i; $j > 0; --$j) {
				if ($rs['genpoly'][$j] != 0) {
					$rs['genpoly'][$j] = $rs['genpoly'][$j-1] ^ $rs['alpha_to'][$this->modnn($rs, $rs['index_of'][$rs['genpoly'][$j]] + $)];
				} else {
					$rs['genpoly'][$j] = $rs['genpoly'][$j-1];
				}
			}
			// rs->genpoly[0] can never be zero
			$rs['genpoly'][0] = $rs['alpha_to'][$this->modnn($rs, $rs['index_of'][$rs['genpoly'][0]] + $)];
		}
	 ...
	 */
	 protected function encode_rs_char($rs, $data, $parity) {
		...
		$GENPOLY  =& $rs['genpoly']; // an array of NS+1 elements containing the generator polynomial in index form
		$NS   =& $rs['ns']; // the number of s in the RS code generator polynomial, which is the same as the number of parity symbols in a block
		$FCR      =& $rs['fcr']; // first consecutive , index form
		$PRIM     =& $rs['prim']; // primitive element, index form
		$IPRIM    =& $rs['iprim']; // prim-th  of 1, index form
		$PAD      =& $rs['pad']; // the number of pad symbols in a block
		$A0       =& $NN;
		$parity = array_fill(0, $NS, 0);
		for ($i=0; $i < ($NN - $NS - $PAD); $i++) {
			$feedback = $INDEX_OF[$data[$i] ^ $parity[0]];
			if ($feedback != $A0) {
				// feedback term is non-zero
				// This line is unnecessary when GENPOLY[NS] is unity, as it must
				// always be for the polynomials constructed by init_rs()
				$feedback = $this->modnn($rs, $NN - $GENPOLY[$NS] + $feedback);
				for ($j=1; $j < $NS; ++$j) {
				$parity[$j] ^= $ALPHA_TO[$this->modnn($rs, $feedback + $GENPOLY[($NS - $j)])];
				}
			}
			// Shift
			array_shift($parity);
