	...
	* @since 5.9.121 (2011-09-28)
	...
	public static function extractCSSproperties($cssdata) {
		...
		foreach ($cssblocks as $block) {
			...
			$c += intval(preg_match_all('/[\:]link|visited|hover|active|focus|target|lang|enabled|disabled|checked|indeterminate||nth|first|last|only|empty|contains|not/i', $selector, $matches)); // number of pseudo-classes
					...
					switch ($attrib[0]) {
						...
						case ':': { // pseudo-class or pseudo-element
							if ($attrib[1] == ':') { // pseudo-element
								...
								// (:, :nth-child(n), :nth-last-child(n), :nth-of-type(n), :nth-last-of-type(n), :first-child, :last-child, :first-of-type, :last-of-type, :only-child, :only-of-type, :empty, :link, :visited, :active, :hover, :focus, :target, :lang(fr), :enabled, :disabled, :checked)
							}
							break;
						}
					} // end of switch
	...
	public static function fileGetContents($file) {
		...
		if ((strlen($file) > 1)
		    && ($file[0] === '/')
		    && ($file[1] !== '/')
		    && !empty($_SERVER['DOCUMENT_'])
		    && ($_SERVER['DOCUMENT_'] !== '/')
		) {
		    $find = strpos($file, $_SERVER['DOCUMENT_']);
		    if (($find === false) || ($find > 1)) {
			$alt[] = htmlspecialchars_decode(urldecode($_SERVER['DOCUMENT_'].$file));
		    }
		}
		//
		$protocol = 'http';
		...
		if (preg_match('%^(https?)://%', $url)
			...
			if (empty($urldata['query'])) {
				...
				if (strpos($url, $host) === 0) {
				    ...
				    $tmp = str_replace($host, $_SERVER['DOCUMENT_'], $url);
