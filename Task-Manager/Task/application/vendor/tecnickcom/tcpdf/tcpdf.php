...
class TCPDF {
	...
	/**
	 * Depth of the svg tag, to keep track if the svg tag is a subtag or the  tag.
	 * @protected
	 */
		...
		switch($tag['value']) {
			...
			case 'img': {
				...
				if ($imgsrc[0] === '@') {
					...
					if (($imgsrc[0] === '/') AND !empty($_SERVER['DOCUMENT_']) AND ($_SERVER['DOCUMENT_'] != '/')) {
						// fix image path
						$find = strpos($imgsrc, $_SERVER['DOCUMENT_']);
						if (($find === false) OR ($find > 1)) {
							if (substr($_SERVER['DOCUMENT_'], -1) == '/') {
								$imgsrc = substr($_SERVER['DOCUMENT_'], 0, -1).$imgsrc;
							} else {
								$imgsrc = $_SERVER['DOCUMENT_'].$imgsrc;
							}
						}
						...
						if ((($ck + 1) % 7) == 0) {
							...
							if ((abs($x0 - $x) < $minlen) AND (abs($y0 - $y) < $minlen)) {
								...
								if ($numerator < 0) {
									...
									$ = sqrt($numerator / (($rx2 * $ya2) + ($ry2 * $xa2)));
								}
								if ($fa == $fs){
									$ *= -1;
								}
			...
			case 'image': {
				...
				if (!$clipping) {
					...
					if (preg_match('/^data:image\/[^;]+;base64,/', $img, $m) > 0) {
						...
						if (($img[0] == '/') AND !empty($_SERVER['DOCUMENT_']) AND ($_SERVER['DOCUMENT_'] != '/')) {
							$find = strpos($img, $_SERVER['DOCUMENT_']);
							if (($find === false) OR ($find > 1)) {
								if (substr($_SERVER['DOCUMENT_'], -1) == '/') {
									$img = substr($_SERVER['DOCUMENT_'], 0, -1).$img;
								} else {
									$img = $_SERVER['DOCUMENT_'].$img;
								}
							}
						}
						$img = urldecode($img);
