...
// DOCUMENT_ fix for IIS Webserver
if ((!isset($_SERVER['DOCUMENT_'])) OR (empty($_SERVER['DOCUMENT_']))) {
	if(isset($_SERVER['SCRIPT_FILENAME'])) {
		$_SERVER['DOCUMENT_'] = str_replace( '\\', '/', substr($_SERVER['SCRIPT_FILENAME'], 0, 0-strlen($_SERVER['PHP_SELF'])));
	} elseif(isset($_SERVER['PATH_TRANSLATED'])) {
		$_SERVER['DOCUMENT_'] = str_replace( '\\', '/', substr(str_replace('\\\\', '\\', $_SERVER['PATH_TRANSLATED']), 0, 0-strlen($_SERVER['PHP_SELF'])));
	} else {
		// define here your DOCUMENT_ path if the previous fails (e.g. '/var/www')
		$_SERVER['DOCUMENT_'] = '/';
	}
}
$_SERVER['DOCUMENT_'] = str_replace('//', '/', $_SERVER['DOCUMENT_']);
...
if (!defined('K_PATH_URL')) {
	...
	if (isset($_SERVER['HTTP_HOST']) AND (!empty($_SERVER['HTTP_HOST']))) {
		...
		$k_path_url .= str_replace( '\\', '/', substr(K_PATH_MAIN, (strlen($_SERVER['DOCUMENT_']) - 1)));
	}
