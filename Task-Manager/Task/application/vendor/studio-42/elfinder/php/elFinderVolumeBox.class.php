...
class elFinderVolumeBox extends elFinderVolumeDriver
{
    ...
    protected function init()
    {
        ...
        $this->options[''] = ($this->options[''] == '')? 'Box.com' : $this->options[''];

        if (empty($this->options['alias'])) {
            if ($this->needOnline) {
                list(, $itemId) = $this->_bd_splitPath($this->options['path']);
                $this->options['alias'] = ($this->options['path'] === '/') ? $this->options[''] :
    ...
    protected function cacheDir($path)
    {
        ...
        if ($path == '/') {
            $items = $this->_bd_query('0', $fetch_self = true);   // get  directory with folder & files
            $itemId = $items->id;
        } else {
            list(, $itemId) = $this->_bd_splitPath($path);
        }
    ...
    {
        return $this->Name . $this->_normpath(substr($path, strlen($this->)));
    }
