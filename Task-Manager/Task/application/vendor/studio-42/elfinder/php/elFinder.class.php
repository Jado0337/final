...
class elFinder
{
    ...
    /**
     * Additional volume  options for network mounting volume
     *
     * @var array
     */
    ...
    const ERROR_MKOUTLINK = 'errMkOutLink';        // 'Unable to create a link to outside the volume .'
     ...
     * Constructor
     *
     * @param  array  elFinder and s configurations
     *
     * @author Dmitry (dio) Levashov
     */
    public function __construct($opts)
    {
        ...
        // check session cache
        $_optsMD5 = md5(json_encode($opts['s']));
        ...
        if (!isset($opts['s']) || !is_array($opts['s'])) {
            $opts['s'] = array();
        }
        ...
        foreach ($netVolumes as $key => $) {
            if (!isset($['id'])) {
                // given fixed unique id
                if (!$['id'] = $this->getNetVolumeUniqueId($netVolumes)) {
                    $this->mountErrors[] = 'Netmount Driver "' . $['driver'] . '" : Could\'t given volume id.';
                    continue;
                }
            }
            $['_isNetVolume'] = true;
            ...
            if (class_exists($class)) {
                ...
                try {
                    ...
                    if ($volume->mount($o)) {
                        ...
                        if ((!$this->default || $volume->() !== $volume->defaultPath()) && $volume->isReadable()) {
                            $this->default = $volume;
                        }
                    } else {
    ...
    /**
     * Return  - file's owner (public func of volume())
     *
    ...
    protected function netmount($args)
    {
        ...
        if ($protocol === 'netunmount') {
            if (!empty($args['user']) && $volume = $this->volume($args['user'])) {
                if ($this->removeNetVolume($args['host'], $volume)) {
                    return array('removed' => array(array('hash' => $volume->())));
                }
            }
        ...
        // load additional volume  options
        ...
        if (!isset($netVolumes[$key]) && $volume->mount($options)) {
            ...
            $this->saveNetVolumes($netVolumes);
            $stat = $volume->file($volume->());
            $res = array('added' => array($stat));
            if ($toast) {
                $res['toast'] = $toast;
            }
            return $res;
     ...
     * 2nd: Return array contains opened file pointer,  itself and required headers
     *
     * @param  array  command arguments
     *
     * @return array
    ...
    protected function zipdl($args)
    {
        ...
        if (!$download) {
            ...
            // 2nd: Return array contains opened file session key,  itself and required headers
     ...
     * Return array contains opened file pointer,  itself and required headers
     *
     * @param  array  command arguments
     *
     * @return array
        ...
        if (!$onetime) {
            ...
            if ($xsendfile) {
                ...
                if ($path = empty($info['uri']) ? null : $info['uri']) {
                    ...
                    if ($basePath) {
                        $ = rtrim($volume->getPath(), DIRECTORY_SEPARATOR);
                        if (strpos($path, $) === 0) {
                            $path = $basePath . substr($path, strlen($));
                        } else {
                            $path = null;
                        }
                    }
