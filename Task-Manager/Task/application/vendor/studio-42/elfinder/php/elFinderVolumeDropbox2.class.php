...
class elFinderVolumeDropbox2 extends elFinderVolumeDriver
{
    ...
    public function __construct()
    {
        $opts = [
            ...
            'separator' => '/',
            'acceptedName' => '#^[^\\\/]+$#',
            'CssClass' => 'elfinder-navbar--dropbox',
            'publishPermission' => [
    ...
    protected function init()
    {
        ...
        if (empty($this->options['alias'])) {
            $this->options['alias'] = sprintf($this->options['aliasFormat'], ($this->options['path'] === '/') ? '' : $this->_basename($this->options['path']));
    ...
    protected function _relpath($path)
    {
        if ($path === $this->) {
            ...
            return ltrim(substr($path, strlen($this->)), '/');
        }
    }
