...
abstract class elFinderVolumeDriver
{
    ...
    protected $options = array(
        ...
        'id' => '',
        // revision id of  directory that uses for caching control of  stat
        'Rev' => '',
        // driver type it uses volume 's CSS class name. e.g. 'group' -> Adds 'elfinder-group' to CSS class name.
        'type' => '',
        //  directory path
        'path' => '',
        ...
        'trashHash' => '',
        // open this path on initial request instead of  path
        'startPath' => '',
        // how many subdirs levels return per request
        'treeDeep' => 1,
        //  url, not set to URL via the connector. If you want to hide the file URL, do not set this value. (replacement for old "fileURL" option)
        'URL' => '',
        ...
        'tmbPathMode' => 0777,
        // thumbnails dir URL. Set it if store thumbnails outside  directory
        'tmbURL' => '',
        // thumbnails size (px)
        'tmbSize' => 48,
        ...
        'uniqueNumFormat' => '%d',
        // list of commands disabled on this 
        'disabled' => array(),
        ...
        //  stat that return without asking the system when mounted and not the current volume. Query to the system with false. array|false
        'rapidStat' => array(
            'read' => true,
            'write' => true,
            'locked' => false,
        ...
        'xsendfile' => '',
        //  path to xsendfile target. Probably, this is required for 'X-Accel-Redirect' on Nginx.
        'xsendfilePath' => ''
    );
    ...
    /**
     * System  path (Unix like: '/', Windows: '\', 'C:\' or 'D:\'...)
     *
     * @var string
     **/
    protected $system = DIRECTORY_SEPARATOR;
     ...
     * @var array
     */
    protected $sessionCaching = array('stat' => true, 'subdirs' => true);
    ...
    /**
     * Is  modified (for clear  stat cache)
     *
     * @var bool
     */
    ...
    public function mount(array $opts)
    {
        ...
        $this-> = $this->normpathCE($this->options['path']);
        $this->separator = isset($this->options['separator']) ? $this->options['separator'] : DIRECTORY_SEPARATOR;
        if (!empty($this->options['winHashFix'])) {
            $this->separatorForHash = ($this->separator !== '/') ? '/' : '';
        }
        $this->system = isset($this->options['system']) ? $this->options['system'] : $this->separator;
        ...
        $this->Name = empty($this->options['alias']) ? $this->basenameCE($this->) : $this->options['alias'];

        // This get's triggered if $this-> == '/' and alias is empty.
        // Maybe modify _basename instead?
        if ($this->Name === '') $this->Name = $this->separator;
        ...
        if (!$) {
            return $this->setError(' folder does not exist.');
        }
        if (!$['read'] && !$['write']) {
            return $this->setError(' folder has not read and write permissions.');
        }

        if ($['read']) {
            if ($argInit) {
                // check startPath - path to open by default instead of 
        ...
        if ($this->tmpLinkPath && !$this->tmpLinkUrl) {
            $cur = realpath('./');
            $i = 0;
            while ($cur !== $this->system && strpos($this->tmpLinkPath, $cur) !== 0) {
                $i++;
                $cur = dirname($cur);
            }
        ...
        if (isset($this->sessionCache['stat'])) {
            unset($this->sessionCache['stat'][$this->getstatCachekey()]);
        }
    ...
    {
        return $this->encode($this->startPath ? $this->startPath : $this->);
    }
    ...
    /**
     * Return file path related to  with convert encoging
     *
     ...
     *
     * @param  string $hash    folder hash or empty string to get tree from  folder
    ...
    {
        $path = $hash ? $this->decode($hash) : $this->;
    ...
    /**
     * Return part of dirs tree from required dir up to  dir
     *
    ...
    public function rename($hash, $name)
    {
        ...
        if (!empty($this->options['netkey']) && !empty($file['is'])) {
            // change alias of netmount 
            $Key = $this->getstatCachekey();
            // delete old cache data
            if ($this->sessionCaching['stat']) {
                unset($this->sessionCaching['stat'][$Key]);
            }
            if (elFinder::$instance->updateNetVolumeOption($this->options['netkey'], 'alias', $name)) {
                $this->clearcache();
                $this->Name = $this->options['alias'] = $name;
    ...
    public function zipdl($hashes)
    {
        ...
        if ($mixed || $this-> == $this->dirnameCE($this->decode($hashes[0]))) {
            $prefix = $this->Name;
        } else {
    ...
    public function search($q, $mimes, $hash = null)
    {
        ...
        // check the leaf  too
        if (!$mimes && (is_null($dir) || $dir == $this->)) {
            $Stat = $this->stat($this->);
            if (!empty($Stat['phash'])) {
                if ($this->stripos($Stat['name'], $q) !== false) {
                    $res = array($Stat);
                }
            }
        }

        return array_merge($res, $this->doSearch(is_null($dir) ? $this-> : $dir, $q, $mimes));
    }
    ...
    public function getContentUrl($hash, $options = array())
    {
        ...
        if (empty($file['url']) && $this->URL) {
            $path = str_replace($this->separator, '/', substr($this->decode($hash), strlen(rtrim($this->, '/' . $this->separator)) + 1));
            if ($this->encoding) {
    ...
    /**
     * Return file path related to  dir (with convert encoding)
     *
     * @param  string $path file path
     *
     * @return string
    ...
    /**
     * Convert path related to  dir into real path (with convert encoding)
     *
    ...
    protected function encode($path)
    {
        if ($path !== '') {
            ...
            // cut  from $path for security reason, even if hacker decodes the path he will not know the 
            $p = $this->relpathCE($path);
            // if reqesting  dir $path will be empty, then assign '/' as we cannot leave it blank for crypt
            if ($p === '') {
                $p = $this->separator;
            }
            // change separator
    ...
    protected function decode($hash)
    {
        if (strpos($hash, $this->id) === 0) {
            ...
            // append  to path after it was cut in encode
            return $this->abspathCE($path);//$this->.($path === $this->separator ? '' : $this->separator.$path);
        }
        return '';
    }
    ...
    /**
     * Return session stat cache key
     *
     * @return string
     */
    protected function getstatCachekey()
    {
        return md5($this-> . (isset($this->options['alias']) ? $this->options['alias'] : ''));
    }
    ...
    protected function stat($path)
    {
        ...
        if ($is_) {
            $Key = $this->getstatCachekey();
            if ($this->sessionCaching['stat'] && !isset($this->sessionCache['stat'])) {
                $this->sessionCache['stat'] = array();
            }
            if (!isset($this->cache[$path]) && !$this->isMyReload()) {
                // need $path as key for netmount/netunmount
                if ($this->sessionCaching['stat'] && isset($this->sessionCache['stat'][$Key])) {
                    if ($ret = $this->sessionCache['stat'][$Key]) {
                        if ($this->options['Rev'] === $ret['Rev']) {
        ...
        if (isset($this->cache[$path])) {
            ...
            if ($is_ && !empty($this->options['rapidStat']) && is_array($this->options['rapidStat']) && !$this->needOnline) {
                $ret = $this->updateCache($path, $this->options['rapidStat'], true);
            } else {
                $ret = $this->updateCache($path, $this->convEncOut($this->_stat($this->convEncIn($path))), true);
                if ($is_ && !empty($Key) && $this->sessionCaching['stat']) {
                    $SessCache = true;
                }
            }
        } 
        if ($is_) {
            if ($ret) {
                $this->Modified = false;
                if ($SessCache) {
                    $this->sessionCache['stat'][$Key] = $ret;
                }
                if (isset($this->options['phash'])) {
                    $ret['is'] = 1;
                    $ret['phash'] = $this->options['phash'];
                }
            } else if (!empty($Key) && $this->sessionCaching['stat']) {
                unset($this->sessionCache['stat'][$Key]);
            }
        }
        return $ret;
    }
    ...
    /**
     * Get  stat extra key values
     *
     * @return array stat extras
     * @author Naoki Sawada
     */
    protected function getStatExtra()
    {
        $stat = array();
        if ($this->Name) {
            $stat['name'] = $this->Name;
        }
        $stat['Rev'] = $this->options['Rev'];
    ...
    protected function updateCache($path, $stat)
    {
        ...
        if ($) {
            $stat = array_merge($stat, $this->getStatExtra());
        } else {
        ...
        if ($stat['read'] && empty($stat['hidden'])) {
            ...
            if (!isset($stat['url']) && $this->URL && $this->encoding) {
                $_path = str_replace($this->separator, '/', substr($path, strlen($this->) + 1));
        ...
        if (!$fromStat && $ && $this->sessionCaching['stat']) {
    ...
    {
        if ($this->Modified || (!empty($this->ARGS['cmd']) && $this->ARGS['cmd'] === 'parents')) {
            return true;
        }
    ...
    protected function updateSubdirsCache($path, $subdirs)
    {
        ...
        if ($this->sessionCaching['stat'] && $path == $this->) {
            unset($this->sessionCache['stat'][$this->getstatCachekey()]);
        }
    }
        ...
        foreach ($this->scandirCE($path) as $p) {
            ...
            if ((!$mimes || $stat['mime'] !== 'directory') && $this->$matchMethod($name, $q, $p) !== false) {
                ...
                if ($this->URL && !isset($stat['url'])) {
                    $path = str_replace($this->separator, '/', substr($p, strlen($this->) + 1));
                    if ($this->encoding) {
    ...
    protected function getFullPath($path, $base)
    {
        ...
        if ($base[0] === $separator && substr($base, 0, strlen($system)) !== $system) {
            $base = $system . substr($base, 1);
        }
        if ($base !== $system) {
        ...
        // Absolute path
        if ($path[0] === $separator || strpos($path, $system) === 0) {
            return $path;
        }
    ...
    /**
     * Return file path related to  dir
     *
     * @param  string $path file path
     *
     * @return string
    ...
    /**
     * Convert path related to  dir into real path
     *
     * @param  string $path rel file path
     *
     * @return string
    ...
    /**
     * Return fake path started from  dir.
     ...
     * - (string) alias   for symlinks - link target path relative to  path. optionally
