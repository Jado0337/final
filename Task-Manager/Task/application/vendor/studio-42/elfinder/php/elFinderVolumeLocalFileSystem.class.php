...
class elFinderVolumeLocalFileSystem extends elFinderVolumeDriver
{
    ...
    {
        $this->options['alias'] = '';              // alias to replace  dir name
        $this->options['dirMode'] = 0755;            // new dirs mode
        $this->options['fileMode'] = 0644;            // new files mode
        $this->options['CssClass'] = 'elfinder-navbar--local';
    ...
    protected function init()
    {
        ...
        if (!isset($this->options['system'])) {
            if ($cwd[0] === DIRECTORY_SEPARATOR || $this->[0] === DIRECTORY_SEPARATOR) {
                $this->system = DIRECTORY_SEPARATOR;
            } else if (preg_match('/^([a-zA-Z]:' . preg_quote(DIRECTORY_SEPARATOR, '/') . ')/', $this->, $m)) {
                $this->system = $m[1];
            } else if (preg_match('/^([a-zA-Z]:' . preg_quote(DIRECTORY_SEPARATOR, '/') . ')/', $cwd, $m)) {
                $this->system = $m[1];
            }
        }
        $this-> = $this->getFullPath($this->, $cwd);
        if (!empty($this->options['startPath'])) {
            $this->options['startPath'] = $this->getFullPath($this->options['startPath'], $this->);
        }
    ...
    protected function configure()
    {
        ...
        if ($['read'] && !$this->tmbURL && $this->URL) {
            if (strpos($this->tmbPath, $this->) === 0) {
                $this->tmbURL = $this->URL . str_replace(DIRECTORY_SEPARATOR, '/', substr($this->tmbPath, strlen($this->) + 1));
    ...
    protected function _joinPath($dir, $name)
    {
        ...
        if ($path === false || strpos($path, $this->) !== 0) {
            if (DIRECTORY_SEPARATOR !== '/') {
    ...
    protected function _relpath($path)
    {
        if ($path === $this->) {
            ...
            if (strpos($path, $this->) === 0) {
                return ltrim(substr($path, strlen($this->)), DIRECTORY_SEPARATOR);
            } else {
                // for link
                return $path;
            }
    ...
    {
        return $this->Name . ($path == $this-> ? '' : $this->separator . $this->_relpath($path));
    }
    ...
    protected function _stat($path)
    {
        ...
        //Verifies the given path is the  or is inside the . Prevents directory traveral.
        if (!$this->_inpath($path, $this->)) {
            return $stat;
        }

        $stat['isowner'] = false;
        $linkreadable = false;
        if ($path != $this-> && is_link($path)) {
        ...
        if (($dir = is_dir($path)) && $this->options['detectDirIcon']) {
            ...
            if ($this->URL && file_exists($favicon)) {
                $stat['icon'] = $this->URL . str_replace(DIRECTORY_SEPARATOR, '/', substr($favicon, strlen($this->) + 1));
            }
        }
    ...
    protected function readlink($path)
    {
        ...
        if (strpos($target, $this->system) !== 0) {
            ...
            foreach ($match as $p) {
                ...
                if ((!$mimes || $stat['mime'] !== 'directory')) {
                    ...
                    if ($this->URL && !isset($stat['url'])) {
                        $_path = str_replace(DIRECTORY_SEPARATOR, '/', substr($p, strlen($this->) + 1));
