...
class elFinderVolumeGoogleDrive extends elFinderVolumeDriver
{
    ...
    public function __construct()
    {
        $opts = [
            ...
            'useGoogleTmb' => true,
            'acceptedName' => '#.#',
            'CssClass' => 'elfinder-navbar--googledrive',
            'publishPermission' => [
                'type' => 'anyone',
                'role' => 'reader',
                'withLink' => true,
    ...
    protected function init()
    {
        ...
        $this-> = $this->options['path'] = $this->_normpath($this->options['path']);

        if (empty($this->options['alias'])) {
            if ($this->needOnline) {
                $this->options[''] = ($this->options[''] === '')? $this->_gd_getNameByPath('') : $this->options[''];
                $this->options['alias'] = ($this->options['path'] === '/') ? $this->options[''] : sprintf($this->options['gdAlias'], $this->_gd_getNameByPath($this->options['path']));
                if (!empty($this->options['netkey'])) {
                    elFinder::$instance->updateNetVolumeOption($this->options['netkey'], 'alias', $this->options['alias']);
                }
            } else {
                $this->options[''] = ($this->options[''] === '')? 'GoogleDrive' : $this->options[''];
                $this->options['alias'] = $this->options[''];
            }
        }

        $this->Name = isset($this->options['alias'])? $this->options['alias'] : 'GoogleDrive';
    ...
    protected function _path($path)
    {
        ...
        $path = $this->_normpath(substr($path, strlen($this->)));
        $names = [];
        $paths = explode('/', $path);
        foreach ($paths as $_p) {
