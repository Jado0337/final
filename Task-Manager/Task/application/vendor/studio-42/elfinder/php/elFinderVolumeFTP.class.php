...
class elFinderVolumeFTP extends elFinderVolumeDriver
{
    ...
    protected function connect()
    {
        ...
        // enter  folder
        if (!ftp_chdir($this->connect, $this->)
            || $this-> != ftp_pwd($this->connect)) {
            $this->umount();
    ...
    protected function _relpath($path)
    {
        if ($path === $this->) {
            ...
            if (strpos($path, $this->) === 0) {
                return ltrim(substr($path, strlen($this->)), $this->separator);
            } else {
                // for link
                return $path;
            }
    ...
    protected function _stat($path)
    {
        ...
        if (!$this->MLSTsupprt) {
            if ($path === $this->) {
                ...
                if ($this->needOnline && (($this->ARGS['cmd'] === 'open' && $this->ARGS['target'] === $this->encode($this->)) || $this->isMyReload())) {
                    $check = array(
                        'ts' => true,
                        'dirs' => true,
                    );
