...
class Utilities extends AdminController
{
    ...
    public function media_connector()
    {
        ...
        if (!is_admin()) {
            ...
            array_push($_options['attributes'], [
            ...
            $_options['path'] = $path;
            $_options['URL']  = site_url($media_folder . '/' . $user->media_path_slug) . '/';
        }

        $publicPath      = $media_folder . '/public';
        $public_         = $_options;
        $public_['path'] = set_realpath($publicPath);

        $public_['URL'] = site_url($media_folder) . '/public';
        unset($public_['attributes'][3]);
