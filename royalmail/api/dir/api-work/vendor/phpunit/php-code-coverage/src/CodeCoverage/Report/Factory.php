...
class PHP_CodeCoverage_Report_Factory
{
    ...
    public function create(PHP_CodeCoverage $coverage)
    {
        ...
        $       = new PHP_CodeCoverage_Report_Node_Directory(
            $commonPath,
            null
        );
    ...
    /**
     * @param PHP_CodeCoverage_Report_Node_Directory $
     * @param array                                  $items
     * @param array                                  $tests
     * @param bool                                   $cacheTokens
     */
    private function addItems(PHP_CodeCoverage_Report_Node_Directory $, array $items, array $tests, $cacheTokens)
    {
        foreach ($items as $key => $value) {
            if (substr($key, -2) == '/f') {
                $key = substr($key, 0, -2);

                if (file_exists($->getPath() . DIRECTORY_SEPARATOR . $key)) {
                    $->addFile($key, $value, $tests, $cacheTokens);
                }
            } else {
                $child = $->addDirectory($key);
